	...
	if ($mode=="modify")
	{
		...
		for ($i=0; $i<$count; $i++)					// update DB with the value of file input box
		{
			...
			if ($del_ok[$i] == "y")
			{
				...
				unlink($delete_path);  //업로드된 파일 삭제명령어
		...
		$sql = "update $table set subject='$subject', about='$about', kind='$kind',content='$content' where num=$num";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	else
	{
		...
		$sql = "insert into $table (id, name, nick, subject, about, kind, content, regist_day, hit, is_html, ";
		$sql .= " file_name_0, file_name_1, file_name_2, file_copied_0,  file_copied_1, file_copied_2) ";
		$sql .= "values('$userid', '$username', '$usernick', '$subject','$about', '$kind', '$content', '$regist_day', 0, '$is_html', ";
